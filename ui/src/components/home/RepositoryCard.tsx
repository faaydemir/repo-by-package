...
export function RepositoryCard({
  ...
  const [extenedPackages, setExtendedPackages] = useState<
    ...
    <div className="flex flex-col gap-1 rounded-sm border border-gray-300 bg-white px-4 py-2">
      <div className="flex items-center justify-between pb-0.5">
        <div className="flex items-center gap-2">
          <Link
            href={url}
            target="_blank"
            className="text-md hover:gray-blue-600 font-semibold text-gray-900"
          >
            {name}
          </Link>
      ...
      {projects.length > 0 &&
        projects
          ...
          .map((project) => (
            <div
              key={project.id}
              className="mt-2 flex flex-col items-start gap-0"
            >
              <Link
              ...
              <div className="w-100 flex flex-wrap items-center justify-start gap-1">
                {project.packages
                  .slice(
                    0,
                    extenedPackages[project.id] ? project.packages.length : 10,
                  )
                  .map((pkg) => (
                ...
                {project.packages.length > 10 && (
                  ...
                  >
                    {extenedPackages[project.id]
                      ? "Show less"
                      : `+ ${project.packages.length - 7} more`}
                  </button>
                )}
              </div>
            </div>
      ...
      {projects.length > 3 && (
        ...
        >
          {showAllProjects
            ? `Show less`
            : `+ ${projects.length - 3} project more.`}
        </button>
      )}
    </div>
  );
