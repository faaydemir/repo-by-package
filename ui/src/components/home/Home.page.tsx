...
export default function Home({ providerId }: Props) {
  ...
  const updateQueryParams = () => {
    ...
    if (state.selectedPackages.length > 0) {
      params.set(
        "packageIds",
        state.selectedPackages.map((p) => p.id).join(","),
      );
    }
    if (
      state.repoSort.field &&
      state.repoSort.field !== defaultState.repoSort.field
    ) {
      params.set("sort", state.repoSort.field);
    }
    if (
      state.repoSort.direction &&
      state.repoSort.direction !== defaultState.repoSort.direction
    ) {
      params.set("direction", state.repoSort.direction);
    }
    if (
      state.repoPagination.page &&
      state.repoPagination.page !== defaultState.repoPagination.page
    ) {
  ...
  const loadInitialState = async (): Promise<void> => {
    const packageIds =
      searchParams.get("packageIds")?.split(",").map(Number) || [];
    const sortField = searchParams.get("sort") || defaultState.repoSort.field;
    const sortDirection =
      searchParams.get("direction") || defaultState.repoSort.direction;
    const page =
      Number(searchParams.get("page")) || defaultState.repoPagination.page;
