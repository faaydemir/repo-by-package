...
export function BackIcon({ size = 5, className = '' }: IconProps) {
	const sizeClasses = `h-${size} w-${size}`;
	return (
		<svg className={`${sizeClasses} ${className}`.trim()} viewBox="0 0 24 24" fill="none" stroke="currentColor">
...
export function StarIcon({ size = 4, className = '' }: IconProps) {
	const sizeClasses = `h-${size} w-${size}`;
	return (
		<svg className={`${sizeClasses} ${className}`.trim()} fill="currentColor" viewBox="0 0 24 24">
