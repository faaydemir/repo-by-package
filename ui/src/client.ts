...
const mergeRepositoriesByProject = (
  repositoryResponses: RepositoryResponseItems[],
): Repository[] => {
...
const searchPackagesById = async (
  packageIds: number[],
...
const generateSearchRepositoriesCacheKey = (
  request: RepositoryFilter,
): string => {
...
const generateCountRepositoriesCacheKey = (
  request: RepositoryCountFilter,
): string => {
  const packageIds = [...(request.packageIds ?? [])];
  packageIds.sort();
...
const generateSearchPackagesCacheKey = (
  request: SearchPackageRequest,
): string => {
